<nav x-data="{ open: false }" class="bg-white border-b border-gray-100">
                        ...
                        <x-jet-dropdown align="right" width="60">
                            <x-slot name="trigger">
                                <span class="inline-flex rounded-md">
                                    <button type="button" class="inline-flex items-center px-3 py-2 text-sm font-medium leading-4 text-gray-500 transition bg-white border border-transparent rounded-md hover:bg-gray-50 hover:text-gray-700 focus:outline-none focus:bg-gray-50 active:bg-gray-50">
                                        {{ optional(optional(Auth::user())->currentTeam)->name }}
                            ...
                            <x-slot name="content">
                                <div class="w-60">
                                    ...
                                    <!-- Team Settings -->
                                    <x-jet-dropdown-link href="{{ route('teams.show', optional(optional(Auth::user())->currentTeam)->id) }}">
                                        {{ __('Team Settings') }}
                                    </x-jet-dropdown-link>
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            ...
            <x-jet-responsive-nav-link href="{{ route('person.index') }}" :active="request()->routeIs('person.index')">
                {{ __('Index') }}
            </x-jet-responsive-nav-link>
            <x-jet-responsive-nav-link href="{{ route('person.create') }}" :active="request()->routeIs('person.create')">
                {{ __('Create') }}
            </x-jet-responsive-nav-link>
        </div>
        ...
        <div class="pt-4 pb-1 border-t border-gray-200">
            <div class="flex items-center px-4">
                @if (Laravel\Jetstream\Jetstream::managesProfilePhotos())
                    <div class="mr-3 shrink-0">
                        <img class="object-cover w-10 h-10 rounded-full" src="{{ Auth::user()->profile_photo_url }}" alt="{{ optional(Auth::user())->name }}" />
                    </div>
                @endif
            ...
            <div class="mt-3 space-y-1">
                ...
                @if (Laravel\Jetstream\Jetstream::hasTeamFeatures())
                    ...
                    <!-- Team Settings -->
                    <x-jet-responsive-nav-link href="{{ route('teams.show', optional(optional(Auth::user())->currentTeam)->id) }}" :active="request()->routeIs('teams.show')">
                        {{ __('Team Settings') }}
                    </x-jet-responsive-nav-link>
